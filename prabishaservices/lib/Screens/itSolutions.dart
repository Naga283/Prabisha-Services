  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: Column(
          children:[
           TopHeader(tit: 'IT Solutions Menu', cartL: _cartList,),
           SizedBox(height: 20,),
                   ...
                   child: GestureDetector(child: (!_cartList.contains(item))
                      ...
                      setState(() {
                        if (!_cartList.contains(item)){
                          _cartList.add(item);
                          ref.doc(firebaseUser?.uid).collection("Cart").add({
                      ...
                      //   ref.doc(firebaseUser?.uid).collection("Cart").doc(snapshot.data!.docs[index].id).delete().whenComplete(() => print("delted"));
